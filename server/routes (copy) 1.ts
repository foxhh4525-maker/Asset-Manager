...
export async function registerRoutes(
  ...
  if (process.env.DISCORD_CLIENT_ID && process.env.DISCORD_CLIENT_SECRET) {
      ...
      new DiscordStrategy(
        {
          ...
          // ✅ تم ضبط الرابط ليكون مطابقاً لإعدادات ديسكورد
          ...
          try {
            ...
            if (!user) {
              user = await storage.createUser({
                ...
                // ✅ تم التغيير إلى admin لضمان دخولك للوحة التحكم
                role: "admin", 
              } as any);
            }
    ...
    // ✅ الحل النهائي لمشكلة الدوران: التوجيه إلى /studio
    app.get(
    ...
    app.get("/api/auth/discord", async (req, res) => {
      ...
      if (!user) {
        user = await storage.createUser({
          ...
          role: "admin", // جعلته أدمن أيضاً للتجربة
        } as any);
      }

      req.login(user, (err) => {
        if (err) return res.status(500).send("Login failed");
        return res.redirect("/studio"); // توجيه للمكان الصحيح
      });
    });
  }
  ...
  app.post(api.clips.create.path, async (req, res) => {
    if (!req.isAuthenticated()) {
      return res
        .status(401)
        .json({ message: "Must be logged in to submit clips" });
    }
